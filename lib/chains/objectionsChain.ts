...
export async function runObjectionsChain(
  ...
  const result = await callStructuredModel(
    `You are a Pakistan/MENAP investor who reviews early-stage startups. You MUST respond with ONLY valid JSON, no markdown, no explanation. Return a JSON object with field topInvestorObjections as an array of exactly 5 objects, each with: objection (string), whyItMatters (string), howToFix (string).`,
