...
export function AgentLog({ entries, totalSteps, startTime }: AgentLogProps) {
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-400 tabular-nums font-medium">{Math.round(displayProgress)}%</span>
      ...
      {activeEntry && !isComplete && (
        <motion.div
          ...
          className="relative rounded-xl border border-gray-200 bg-white p-4 overflow-hidden"
        >
          <div className="absolute inset-0 bg-gradient-to-r from-orange-50 to-amber-50 animate-pulse" />
          <div className="relative flex items-center gap-3">
            <div className={`w-8 h-8 rounded-lg bg-gradient-to-br ${AGENT_COLORS[activeEntry.agent] || "from-orange-500 to-amber-500"} flex items-center justify-center`}>
              <Bot className="w-4 h-4 text-white" />
            </div>
            <div className="flex-1">
              <p className="text-sm font-medium text-gray-900">{activeEntry.agent}</p>
              <p className="text-sm text-gray-500">{activeEntry.message}</p>
            </div>
      ...
      <div
        ref={logRef}
        className="rounded-xl border border-gray-200 bg-gray-50 max-h-[300px] overflow-y-auto"
      >
        <div className="p-4 space-y-0">
          <AnimatePresence initial={false}>
            {entries.map((entry, i) => {
              const isLast = i === entries.length - 1;
              const gradient = AGENT_COLORS[entry.agent] || "from-orange-500 to-amber-500";
              return (
                <motion.div
                  key={`${entry.step}-${i}`}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {entry.insight && (
                      <p className="text-sm text-orange-500/70 mt-1">{entry.insight}</p>
                    )}
